...
METRICS_PULL_ERROR ?=
GODOT_BIN ?= $(shell if [ -x "$(PWD)/.tools/godot/current/Godot.app/Contents/MacOS/Godot" ]; then printf "%s" "$(PWD)/.tools/godot/current/Godot.app/Contents/MacOS/Godot"; else printf "godot"; fi)
GODOT_SCRIPT ?=
GODOT_SECONDS ?= 5
GODOT_FPS ?= 30
GODOT_MAX_NODES ?= 200
